...
# ChatGPT (5.2 Thinking model) was used to implement stubbed functions. Such stubs had their
# function defined in detail in a documentation comment (which hasn't been changed) to guide the AI
# model.

# Guidance provided to the AI for all function implementation was as follows:
...
class Crawler:
    def __init__(self, seeds, politeness_window=6, crawl_limit=1000):
    ...
    def crawl():
        """ This function begins the crawling process. It continously scans the frontier for hosts that can be crawled, and 
            makes use of the download_web_page and parse_web_page functions to perform the crawl. It ensures that concurrent requests to the same host are spaced by the politeness_window; and that we don't crawl
            any more pages than the crawl_limit.
        """
